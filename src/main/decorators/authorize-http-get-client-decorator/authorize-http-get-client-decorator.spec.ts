...
describe('AuthorizeHttpGetClientDecorator', () => {
  test('Shoul call GetStorage with correct value', async () => {
    const { sut, getStorageSpy } = makeSut()
    await sut.get(mockGetRequest())
    expect(getStorageSpy.key).toBe('account')
  })

  test('Shoul not add headers if GetStorage is invalid', async () => {
  ...
  test('Shoul add headers to HttpGetClient', async () => {
